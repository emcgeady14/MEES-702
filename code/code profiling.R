...
library(snowboot)
#function for sampling on a network, snowball sampling
#widely used in social science research, tracing HIV contact etc.
...
#tests nested coils, whether xt can be predicted by its lag + variable
#appearance of rain predicts mud on road
library(funtimes)
...
# See the time it takes
#using once but using 1000L bootstrap samples so it will take longer
system.time(
...
d = summaryRprof(tmp)  #save model so you can access coeff without having to rerun
...
#opens up window that you can toggle over

#good tools to find out where to optimize a function ^ HW2


# parallel ----
#base R, but must load it
library(parallel)
...
) #takes a while - 8 seconds

# Same operation executed in parallel
library(parallel)
# Create cluster to be faster
cl <- parallel::makeCluster(parallel::detectCores())  #I have 22 cores!! 
#or replace "parallel::detectCores()" with how many you want to use
...
) #this was way faster! 0.72 seconds

#improvement:
8.03/0.72 #11.15 times faster
...
#look into these references to figure out how to speed up functions
# References ----
